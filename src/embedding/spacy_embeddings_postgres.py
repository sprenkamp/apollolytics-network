...
BATCH_SIZE = 10000  # You can adjust this value as needed
...
async def add_embeddings():
    ...
    # Handle column name to be safe for SQL
    # Extract model name after the last slash and replace special characters
    column_name = f"embedding_{embedding_model_name.split('/')[-1].replace('-', '_')}"
                        ...
                        async with connection.transaction():
                            for row in batch:
                                try:
                                    ...
                                    embedding = model.encode(row["messagetext"], convert_to_numpy=True).tolist()
